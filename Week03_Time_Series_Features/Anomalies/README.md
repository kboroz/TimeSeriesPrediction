# Anomalies:

## Anomaly detection in time series with Python

https://www.youtube.com/watch?v=qy41dXGbAxY

## AnomalyDetectionNixtla

https://github.com/PieroPaialungaAI/AnomalyDetectionNixtla

## Dynamic time warping (DTW)

https://www.youtube.com/playlist?list=PLmZlBIcArwhMJoGk5zpiRlkaHUqy5dLzL


# Additional linls and references:

## Anomaly Detection in Time Series Using Statistical Analysis

https://medium.com/booking-com-development/anomaly-detection-in-time-series-using-statistical-analysis-cc587b21d008

## An introduction to Dynamic Time Warping

https://rtavenar.github.io/blog/dtw.html

## Dynamic time warping

https://en.wikipedia.org/wiki/Dynamic_time_warping#References

## Dynamic Time Warp suite!

https://dynamictimewarping.github.io/

## gaitmap - The Gait and Movement Analysis Package

https://gaitmap.readthedocs.io/en/v2.5.1/README.html

https://gaitmap.readthedocs.io/en/v2.5.1/auto_examples/stride_segmentation/index.html

## tslearn is a Python package that provides machine learning tools for the analysis of time series

https://tslearn.readthedocs.io/en/stable/auto_examples/index.html

## TranAD: Deep Transformer Networks for Anomaly Detection in Multivariate Time Series Data

https://github.com/imperial-qore/TranAD

## Anomaly Detection with AutoEncoders using Tensorflow

https://www.youtube.com/watch?v=S31E-ftRfQI&list=PL3N9eeOlCrP5cK0QRQxeJd6GrQvhAtpBK&index=23

## Anomaly detection using Isolation Forest - Contextual Anomalies

https://www.youtube.com/watch?v=XCF-kqCB_vA&list=PL3N9eeOlCrP5cK0QRQxeJd6GrQvhAtpBK&index=33

## Exercise: Anomaly Detection

https://ml-lectures.org/docs/unsupervised_learning/Anomaly_Detection_RNN_AE_VAE.html

## ANOMALY DETECTION FOR TIME SERIES USING VAE-LSTM HYBRID MODEL

https://oxford-man.ox.ac.uk/wp-content/uploads/2020/06/ANOMALY-DETECTION-FOR-TIME-SERIES-USING-VAE-LSTM-HYBRID-MODEL.pdf

## LSTM-Based VAE-GAN for Time-Series Anomaly Detection

https://www.mdpi.com/1424-8220/20/13/3738

## Timeseries clustering

https://github.com/tejaslodaya/timeseries-clustering-vae

## Anomagram: Interactive Visualization for Autoencoders with Tensorflow.js

https://github.com/victordibia/anomagram/

## TODS: Automated Time-series Outlier Detection System

https://github.com/datamllab/tods

## PyOD, a comprehensive but easy-to-use Python library for detecting anomalies in multivariate data

https://pyod.readthedocs.io/en/latest/index.html
